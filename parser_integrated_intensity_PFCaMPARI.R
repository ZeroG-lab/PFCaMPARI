#Parser_red_integrated_intenstiy_PFCaMPARI
#Load libraries
library(readxl)
...
for (k in 1:3) {
  ...
  for (i in 1:8) {
    #read data
    df_Flight <- read_xlsx(paste0("Integrated_intensity_Flight_", k, ".xlsx"), sheet = i)
    #cut first 6 rows 
    ...
    colnames(Flight)[1:2] <- c("Well", "IntegratedIntensity")
...
# col_order <- c("row","col","Well","IntegratedIntensity","Flight","Unit", "Board", "Treatment")
