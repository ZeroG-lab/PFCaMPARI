...
#for (i in p) {
  ...
  boxplot <- ggplot(subset, aes(x=phase, y=Green_Red_Mean_Intensity_norm_Green_Mean_Intensity, fill=Condition)) + 
    ...
    ggtitle(paste0("Converted Cells Red Mean Intensity/Green Mean Intensity"))
  
  figure <- boxplot + stat_compare_means(label = "p.signif", method = "t.test",
                               ref.group = "Pre-Parabola 1",
                               label.y = 0.06, hide.ns = TRUE)
  
  #save as vector graphic in .eps format
  ggsave(filename = paste0("Converted Cells Red Mean Intensity_Green Mean Intensity",".png"),
         plot = print(figure),
         device = png())
#}
