...
#subset for parabola and construct
  ...
  subset <- subset(PFC_Merged, Construct == "CaMPARI2-F391W" & phase  == "Pre-Parabola 2") 
  ...
  boxplot <- ggplot(subset, aes(x=as.factor(Parabola), y=ConversionRate, fill=as.factor(Parabola))) + 
    geom_boxplot() +
    theme_light()+
    theme(
    ...
    ggtitle(paste0("Conversion rates, parabola: ",p[i]))
